...
// ─── QUIZ LESSON ───
const QUESTIONS = [
  { q: "¿Qué método HTTP usás para CREAR un recurso nuevo?", opts: ["GET", "POST", "PUT", "DELETE"], correct: 1, explain: "POST crea recursos nuevos. Cada POST puede crear un recurso diferente (no es idempotente)." },
  { q: "Recibís un error 401. ¿Qué significa?", opts: ["El recurso no existe", "No tenés autenticación", "El servidor falló", "Rate limit"], correct: 1, explain: "401 = Unauthorized. No proporcionaste credenciales válidas. Diferente de 403 (tenés credenciales pero no permiso)." },
  { q: "¿Cuál de estos JSON es VÁLIDO?", opts: ["{ nombre: 'test' }", '{ "nombre": "test" }', "{ 'nombre': 'test' }", '{ "nombre": test }'], correct: 1, explain: 'En JSON, las keys y strings SIEMPRE van con comillas dobles (").' },
  { q: "¿Qué header se usa para indicar el formato del body?", opts: ["Authorization", "Content-Type", "Accept", "User-Agent"], correct: 1, explain: "Content-Type dice qué formato ENVÍAS (ej: application/json). Accept dice qué formato QUERÉS recibir." },
  { q: "Error 404 vs 403: ¿cuál es la diferencia?", opts: ["Son lo mismo", "404=no existe, 403=sin permiso", "403=no existe, 404=sin permiso", "Ambos son del servidor"], correct: 1, explain: "404 = el recurso no existe. 403 = existe pero no tenés permiso para accederlo." },
  { q: "¿Cuál es la diferencia entre PUT y PATCH?", opts: ["No hay diferencia", "PUT reemplaza todo, PATCH solo partes", "PATCH es más rápido", "PUT es para crear"], correct: 1, explain: "PUT reemplaza el recurso completo. PATCH actualiza solo los campos que enviás. PUT necesita el objeto completo." },
  { q: "¿Qué significa que GET es 'idempotente'?", opts: ["Es más rápido", "Llamarlo N veces da el mismo resultado", "Solo funciona una vez", "No modifica datos"], correct: 1, explain: "Idempotente = llamarlo 1 vez o 100 da el mismo resultado. GET, PUT y DELETE son idempotentes. POST no." },
  { q: "Recibís un 429. ¿Qué hacés?", opts: ["Reintentar inmediatamente", "Esperar y reintentar (backoff)", "Cambiar de endpoint", "Borrar el token"], correct: 1, explain: "429 = Too Many Requests (rate limit). La estrategia correcta es exponential backoff: esperar cada vez más entre reintentos." },
];
