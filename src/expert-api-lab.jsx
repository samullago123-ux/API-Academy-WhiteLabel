...
// ─── QUIZ ───
const ALL_QUESTIONS = [
  { q: "¿Qué nivel del modelo de Richardson usa HATEOAS?", opts: ["Nivel 0", "Nivel 1", "Nivel 2", "Nivel 3"], correct: "Nivel 3", explain: "Nivel 3 agrega Hypermedia As The Engine Of Application State: el response incluye links de navegación que dicen qué acciones están disponibles." },
  { q: "¿Cuál es la principal ventaja de GraphQL sobre REST?", opts: ["Es más rápido", "Evita over-fetching y under-fetching", "Es más seguro", "Es más fácil de cachear"], correct: "Evita over-fetching y under-fetching", explain: "GraphQL te deja pedir exactamente los campos que necesitás en una sola query, sin recibir datos de más ni hacer múltiples requests." },
  { q: "En el Teorema CAP, ¿qué se sacrifica en un sistema AP (Disponibilidad + Partición)?", opts: ["Disponibilidad", "Consistencia", "Velocidad", "Seguridad"], correct: "Consistencia", explain: "AP sacrifica consistencia: el sistema sigue respondiendo durante particiones de red, pero dos nodos pueden dar datos distintos temporalmente (consistencia eventual)." },
  { q: "¿Qué es una Saga en sistemas distribuidos?", opts: ["Un tipo de base de datos", "Una secuencia de transacciones con compensaciones en caso de falla", "Un protocolo de autenticación", "Un patrón de caché"], correct: "Una secuencia de transacciones con compensaciones en caso de falla", explain: "Las Sagas reemplazan las transacciones distribuidas: si un paso falla, se ejecutan compensaciones en reversa para deshacer los pasos anteriores." },
  { q: "¿Qué es BOLA según OWASP API Security?", opts: ["Un tipo de ataque DDoS", "Broken Object-Level Authorization — acceder a recursos de otros cambiando el ID", "Un protocolo de encriptación", "Un método de autenticación"], correct: "Broken Object-Level Authorization — acceder a recursos de otros cambiando el ID", explain: "BOLA es la vulnerabilidad #1 en APIs: cambiar /users/1 a /users/2 y acceder a datos ajenos porque no se verifica el ownership del recurso." },
  { q: "¿Qué estrategia de cache lee la DB solo cuando hay cache miss?", opts: ["Write-Through", "Write-Behind", "Cache-Aside (Lazy)", "CDN Edge"], correct: "Cache-Aside (Lazy)", explain: "Cache-Aside: la app revisa cache primero → si HIT, devuelve → si MISS, lee DB, guarda en cache, devuelve. Es el patrón más común con Redis." },
  { q: "¿Para qué sirve OpenAPI (Swagger)?", opts: ["Solo para documentación", "Es un framework backend", "Describir el contrato de tu API para generar docs, SDKs, mocks y validación", "Es un tipo de base de datos"], correct: "Describir el contrato de tu API para generar docs, SDKs, mocks y validación", explain: "OpenAPI es un spec que describe tu API. Desde ese archivo se generan docs (Swagger UI), SDKs (openapi-generator), mocks, y validación de requests." },
  { q: "¿Cuál es la diferencia entre Message Queue y Pub/Sub?", opts: ["Son lo mismo", "Queue: un mensaje → un consumidor. Pub/Sub: un evento → todos los suscriptores", "Pub/Sub es más rápido", "Queue es para archivos, Pub/Sub para texto"], correct: "Queue: un mensaje → un consumidor. Pub/Sub: un evento → todos los suscriptores", explain: "Message Queue: cada mensaje se procesa UNA vez por UN worker. Pub/Sub: un evento se envía a TODOS los servicios suscritos." },
  { q: "¿Por qué CORS solo afecta al browser y no a Postman/curl/n8n?", opts: ["Es un bug de los browsers", "CORS es una policy del browser que restringe requests cross-origin; backends la ignoran", "Postman tiene permisos especiales", "CORS solo aplica a JavaScript"], correct: "CORS es una policy del browser que restringe requests cross-origin; backends la ignoran", explain: "CORS es un mecanismo de seguridad implementado por los browsers. Herramientas como Postman, curl, y n8n no lo implementan porque no ejecutan JavaScript en un origen web." },
  { q: "¿Qué patrón permite que el frontend trabaje antes de que el backend esté listo?", opts: ["Cache-Aside", "API-First con mock server desde OpenAPI spec", "Event Sourcing", "Circuit Breaker"], correct: "API-First con mock server desde OpenAPI spec", explain: "Con API-First, definís el spec OpenAPI primero, generás un mock server, y el frontend trabaja contra ese mock mientras el backend implementa la API real en paralelo." },
  { q: "En Event Sourcing, ¿cómo se obtiene el estado actual?", opts: ["Se lee directamente de la DB", "Se reproduce la secuencia completa de eventos", "Se cachea permanentemente", "Se calcula con GraphQL"], correct: "Se reproduce la secuencia completa de eventos", explain: "En Event Sourcing no guardás el estado actual — guardás todos los eventos. El estado se reconstruye reproduciendo los eventos en orden. Como un libro contable." },
  { q: "¿Qué es Mass Assignment?", opts: ["Asignar memoria en exceso", "El cliente envía campos que no debería poder modificar (como role: admin) y el server los acepta", "Un tipo de base de datos", "Un protocolo de red"], correct: "El cliente envía campos que no debería poder modificar (como role: admin) y el server los acepta", explain: "Mass Assignment ocurre cuando hacés db.update(req.body) sin whitelist. El atacante puede enviar { role: 'admin' } y el server lo acepta ciegamente." },
  { q: "¿Cuándo NO deberías usar GraphQL?", opts: ["Cuando tenés relaciones complejas", "Cuando necesitás caché HTTP agresivo y tu API es pública con operaciones CRUD simples", "Cuando el frontend es React", "Cuando tenés muchos datos"], correct: "Cuando necesitás caché HTTP agresivo y tu API es pública con operaciones CRUD simples", explain: "GraphQL complica el caching HTTP (todo va a POST /graphql). Para APIs públicas con CRUD simple y caché agresivo, REST es más eficiente." },
  { q: "¿Qué significa 'polyglot persistence'?", opts: ["Hablar múltiples idiomas", "Usar la base de datos correcta para cada tipo de dato", "Una base de datos que soporta múltiples lenguajes", "Un ORM universal"], correct: "Usar la base de datos correcta para cada tipo de dato", explain: "Polyglot persistence: PostgreSQL para transacciones, Redis para cache, Elasticsearch para búsqueda, S3 para archivos. Cada dato va donde mejor se maneja." },
  { q: "¿Qué es un BFF (Backend for Frontend)?", opts: ["Un framework JavaScript", "Un servicio intermedio que adapta la API para cada tipo de cliente", "Un tipo de cache", "Un protocolo de red"], correct: "Un servicio intermedio que adapta la API para cada tipo de cliente", explain: "BFF: un servicio que adapta la API general para las necesidades específicas de cada cliente (web vs mobile vs WhatsApp). Evita que un solo endpoint sirva a todos mal." },
  { q: "¿Por qué es importante que los servicios sean stateless?", opts: ["Son más baratos", "Permiten escalar horizontalmente — cualquier instancia puede manejar cualquier request", "Son más seguros", "Son más rápidos"], correct: "Permiten escalar horizontalmente — cualquier instancia puede manejar cualquier request", explain: "Si un servicio no guarda estado en memoria, podés tener 10 instancias detrás de un load balancer. Cualquiera puede atender cualquier request." },
  { q: "¿Qué es contract testing?", opts: ["Testing de contratos legales", "Verificar que tu implementación cumple el spec OpenAPI en cada deploy", "Testing de APIs de pago", "Testing de performance"], correct: "Verificar que tu implementación cumple el spec OpenAPI en cada deploy", explain: "Contract testing: en cada PR/deploy, un test automático verifica que tus endpoints devuelven exactamente lo que dice el spec OpenAPI. Si rompés el contrato, el test falla." },
  { q: "Un campo nuevo en el response de tu API, ¿es un breaking change?", opts: ["Sí, siempre", "No, agregar campos nuevos nunca rompe clientes existentes", "Solo si es un array", "Depende del nombre del campo"], correct: "No, agregar campos nuevos nunca rompe clientes existentes", explain: "Agregar un campo nuevo es un non-breaking change. Los clientes existentes simplemente lo ignoran. Remover o renombrar un campo SÍ es breaking." },
  { q: "¿Qué hace kebab-case en URLs de APIs?", opts: ["/getUserOrders", "/user-orders (separado por guiones)", "/user_orders", "/USER_ORDERS"], correct: "/user-orders (separado por guiones)", explain: "Las URLs de APIs profesionales usan kebab-case: /order-items, /user-profiles. Es el estándar porque las URLs son case-insensitive en la práctica." },
  { q: "¿Qué es consistencia eventual?", opts: ["Los datos siempre están consistentes", "Después de un write, todos los nodos eventualmente convergen al mismo estado", "Un tipo de base de datos", "Un protocolo de seguridad"], correct: "Después de un write, todos los nodos eventualmente convergen al mismo estado", explain: "Consistencia eventual: después de escribir, puede haber un momento donde distintos nodos tienen datos distintos, pero eventualmente todos convergen. Tradeoff de sistemas AP." },
];
